...
type DescribeSpotPriceHistoryInput struct {
	...
	// the Spot price (Linux/UNIX | Red Hat Enterprise Linux | SUSE Linux | Windows |
	// Linux/UNIX (Amazon VPC) | Red Hat Enterprise Linux (Amazon VPC) | SUSE Linux
	// (Amazon VPC) | Windows (Amazon VPC)).
	//
	//     * spot-price - The Spot price. The
	// value must match exactly (or use wildcards; greater than or less than comparison
	// is not supported).
	//
	//     * timestamp - The time stamp of the Spot price history,
	// in UTC format (for example, YYYY-MM-DDTHH:MM:SSZ). You can use wildcards (* and
...
func addOperationDescribeSpotPriceHistoryMiddlewares(stack *middleware.Stack, options Options) (err error) {
	err = stack.Serialize.Add(middleware.After, &awsEc2query_serializeOpDescribeSpotPriceHistory{})
	if err != nil {
		return err
	}
	err = stack.Deserialize.Add(middleware.After, &awsEc2query_deserializeOpDescribeSpotPriceHistory{})
	if err != nil {
		return err
	}
	...
	if err := stack.Initialize.Add(middleware.Before, newServiceMetadataMiddleware_opDescribeSpotPriceHistory(options.Region)); err != nil {
		return err
	}
