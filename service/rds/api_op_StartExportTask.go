...
type StartExportTaskInput struct {
	...
	// KMS key alias for the KMS encryption key. The caller of this operation must be
	// authorized to execute the following operations. These can be set in the KMS key
	// policy:
	//
...
func addOperationStartExportTaskMiddlewares(stack *middleware.Stack, options Options) (err error) {
	err = stack.Serialize.Add(middleware.After, &awsAwsquery_serializeOpStartExportTask{})
	if err != nil {
		return err
	}
	err = stack.Deserialize.Add(middleware.After, &awsAwsquery_deserializeOpStartExportTask{})
	if err != nil {
		return err
	}
	...
	if err := stack.Initialize.Add(middleware.Before, newServiceMetadataMiddleware_opStartExportTask(options.Region)); err != nil {
		return err
	}
