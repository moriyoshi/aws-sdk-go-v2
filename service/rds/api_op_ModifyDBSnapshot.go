...
// upgrading DB snapshots for MySQL, Oracle, and PostgreSQL.
func (c *Client) ModifyDBSnapshot(ctx context.Context, params *ModifyDBSnapshotInput, optFns ...func(*Options)) (*ModifyDBSnapshotOutput, error) {
	...
	result, metadata, err := c.invokeOperation(ctx, "ModifyDBSnapshot", params, optFns, addOperationModifyDBSnapshotMiddlewares)
	if err != nil {
		return nil, err
	}
...
type ModifyDBSnapshotInput struct {
	...
	//
	//     * 11.2.0.4.v12 (supported for
	// 11.2.0.2 DB snapshots)
	//
	//     * 11.2.0.4.v11 (supported for 11.2.0.3 DB
	// snapshots)
	//
	// PostgreSQL For the list of engine versions that are available for
	// upgrading a DB snapshot, see  Upgrading the PostgreSQL DB Engine for Amazon RDS
...
func addOperationModifyDBSnapshotMiddlewares(stack *middleware.Stack, options Options) (err error) {
	err = stack.Serialize.Add(middleware.After, &awsAwsquery_serializeOpModifyDBSnapshot{})
	if err != nil {
		return err
	}
	err = stack.Deserialize.Add(middleware.After, &awsAwsquery_deserializeOpModifyDBSnapshot{})
	if err != nil {
		return err
	}
	...
	if err := stack.Initialize.Add(middleware.Before, newServiceMetadataMiddleware_opModifyDBSnapshot(options.Region)); err != nil {
		return err
	}
