...
//
// You cannot
// specify access permission using both the body and the request headers. Depending
// on your application needs, you may choose to set the ACL on a bucket using
// either the request body or the headers. For example, if you have an existing
// application that updates a bucket ACL using the request body, then you can
// continue to use that approach. Access Permissions
//
// You can set access
// permissions using one of the following methods:
//
//     * Specify a canned ACL with
// the x-amz-acl request header. Amazon S3 supports a set of predefined ACLs, known
// as canned ACLs. Each canned ACL has a predefined set of grantees and
...
// General Reference.
//
//     For example, the following x-amz-grant-write header
...
func addOperationPutBucketAclMiddlewares(stack *middleware.Stack, options Options) (err error) {
	err = stack.Serialize.Add(middleware.After, &awsRestxml_serializeOpPutBucketAcl{})
	if err != nil {
		return err
	}
	err = stack.Deserialize.Add(middleware.After, &awsRestxml_deserializeOpPutBucketAcl{})
	if err != nil {
		return err
	}
	...
	if err := stack.Initialize.Add(middleware.Before, newServiceMetadataMiddleware_opPutBucketAcl(options.Region)); err != nil {
		return err
	}
