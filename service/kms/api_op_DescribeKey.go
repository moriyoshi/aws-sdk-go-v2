...
//
// If you call the
// DescribeKey operation on a predefined AWS alias, that is, an AWS alias with no
// key ID, AWS KMS creates an AWS managed CMK
...
func addOperationDescribeKeyMiddlewares(stack *middleware.Stack, options Options) (err error) {
	err = stack.Serialize.Add(middleware.After, &awsAwsjson11_serializeOpDescribeKey{})
	if err != nil {
		return err
	}
	err = stack.Deserialize.Add(middleware.After, &awsAwsjson11_deserializeOpDescribeKey{})
	if err != nil {
		return err
	}
	...
	if err := stack.Initialize.Add(middleware.Before, newServiceMetadataMiddleware_opDescribeKey(options.Region)); err != nil {
		return err
	}
