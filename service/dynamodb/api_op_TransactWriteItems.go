...
// met.
//
//     * Delete  Initiates a DeleteItem operation to delete an existing
// item. This structure specifies the primary key of the item to be deleted, the
// name of the table where it resides, an optional condition expression that must
// be satisfied for the deletion to succeed, and a field indicating whether to
...
//
// DynamoDB rejects
// the entire TransactWriteItems request if any of the following is true:
//
//     * A
// condition in one of the condition expressions is not met.
//
//     * An ongoing
// operation is in the process of updating the same item.
//
//     * There is
...
func addOperationTransactWriteItemsMiddlewares(stack *middleware.Stack, options Options) (err error) {
	err = stack.Serialize.Add(middleware.After, &awsAwsjson10_serializeOpTransactWriteItems{})
	if err != nil {
		return err
	}
	err = stack.Deserialize.Add(middleware.After, &awsAwsjson10_deserializeOpTransactWriteItems{})
	if err != nil {
		return err
	}
	...
	if err := stack.Initialize.Add(middleware.Before, newServiceMetadataMiddleware_opTransactWriteItems(options.Region)); err != nil {
		return err
	}
...
func addIdempotencyToken_opTransactWriteItemsMiddleware(stack *middleware.Stack, cfg Options) error {
	return stack.Initialize.Add(middleware.Before, &idempotencyToken_initializeOpTransactWriteItems{tokenProvider: cfg.IdempotencyTokenProvider})
}
