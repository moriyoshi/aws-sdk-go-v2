...
// account-level quotas. The per-table quotas apply only when you are creating a
...
// the only quota that applies is that the aggregate provisioned capacity over all
// your tables and GSIs cannot exceed either of the per-account quotas.
...
func addOperationDescribeLimitsMiddlewares(stack *middleware.Stack, options Options) (err error) {
	err = stack.Serialize.Add(middleware.After, &awsAwsjson10_serializeOpDescribeLimits{})
	if err != nil {
		return err
	}
	err = stack.Deserialize.Add(middleware.After, &awsAwsjson10_deserializeOpDescribeLimits{})
	if err != nil {
		return err
	}
	...
	if err := stack.Initialize.Add(middleware.Before, newServiceMetadataMiddleware_opDescribeLimits(options.Region)); err != nil {
		return err
	}
