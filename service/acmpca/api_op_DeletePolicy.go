...
// account.
//
//     * For ACM to manage automatic renewal of these certificates, the
// ACM user must configure a Service Linked Role (SLR). The SLR allows the ACM
// service to assume the identity of the user, subject to confirmation against the
// ACM Private CA policy. For more information, see Using a Service Linked Role
// with ACM (https://docs.aws.amazon.com/acm/latest/userguide/acm-slr.html).
//
//     *
...
func addOperationDeletePolicyMiddlewares(stack *middleware.Stack, options Options) (err error) {
	err = stack.Serialize.Add(middleware.After, &awsAwsjson11_serializeOpDeletePolicy{})
	if err != nil {
		return err
	}
	err = stack.Deserialize.Add(middleware.After, &awsAwsjson11_deserializeOpDeletePolicy{})
	if err != nil {
		return err
	}
	...
	if err := stack.Initialize.Add(middleware.Before, newServiceMetadataMiddleware_opDeletePolicy(options.Region)); err != nil {
		return err
	}
