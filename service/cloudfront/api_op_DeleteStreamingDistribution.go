...
//
//     * Submit a
// GET Streaming Distribution Config request to get the current configuration and
// the Etag header for the distribution.
//
//     * Update the XML document that was
// returned in the response to your GET Streaming Distribution Config request to
// change the value of Enabled to false.
//
//     * Submit a PUT Streaming Distribution
// Config request to update the configuration for your distribution. In the request
// body, include the XML document that you updated in Step 3. Then set the value of
// the HTTP If-Match header to the value of the ETag header that CloudFront
...
// Deployed.
//
//     * Submit a DELETE Streaming Distribution request. Set the value
...
//
// For information
// about deleting a distribution using the CloudFront console, see Deleting a
// Distribution
...
func addOperationDeleteStreamingDistributionMiddlewares(stack *middleware.Stack, options Options) (err error) {
	err = stack.Serialize.Add(middleware.After, &awsRestxml_serializeOpDeleteStreamingDistribution{})
	if err != nil {
		return err
	}
	err = stack.Deserialize.Add(middleware.After, &awsRestxml_deserializeOpDeleteStreamingDistribution{})
	if err != nil {
		return err
	}
	...
	if err := stack.Initialize.Add(middleware.Before, newServiceMetadataMiddleware_opDeleteStreamingDistribution(options.Region)); err != nil {
		return err
	}
