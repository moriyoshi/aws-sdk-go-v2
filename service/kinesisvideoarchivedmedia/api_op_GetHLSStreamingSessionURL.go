...
//
//     * Provide the URL (containing the encrypted
// session token) for the HLS master playlist to a media player that supports the
// HLS protocol. Kinesis Video Streams makes the HLS media playlist, initialization
// fragment, and media fragments available through the master playlist URL. The
...
// and resolution.
//
//         * GetHLSMediaPlaylist: Retrieves an HLS media playlist,
...
// GetMP4MediaFragment actions. The HLS media playlist also contains metadata about
// the stream that the player needs to play it, such as whether the PlaybackMode is
// LIVE or ON_DEMAND. The HLS media playlist is typically static for sessions with
// a PlaybackType of ON_DEMAND. The HLS media playlist is continually updated with
// new fragments for sessions with a PlaybackType of LIVE. There is a distinct HLS
...
type GetHLSStreamingSessionURLInput struct {
	...
	// NEVER to ensure the media player timeline most accurately maps to the producer
	// timestamps.
	//
	...
	// playback scenarios, it is recommended to use a value of ON_DISCONTINUITY so that
	// the media player timeline is only reset when there is a significant issue with
	...
	//
	//     * ON_DEMAND : For sessions of
	// this type, the HLS media playlist contains all the fragments for the session, up
	// to the number that is specified in MaxMediaPlaylistFragmentResults. The playlist
...
func addOperationGetHLSStreamingSessionURLMiddlewares(stack *middleware.Stack, options Options) (err error) {
	err = stack.Serialize.Add(middleware.After, &awsRestjson1_serializeOpGetHLSStreamingSessionURL{})
	if err != nil {
		return err
	}
	err = stack.Deserialize.Add(middleware.After, &awsRestjson1_deserializeOpGetHLSStreamingSessionURL{})
	if err != nil {
		return err
	}
	...
	if err := stack.Initialize.Add(middleware.Before, newServiceMetadataMiddleware_opGetHLSStreamingSessionURL(options.Region)); err != nil {
		return err
	}
