...
func AddAcceptEncodingGzip(stack *middleware.Stack, options AddAcceptEncodingGzipOptions) error {
	if options.Enable {
		if err := stack.Finalize.Add(middleware.Before, &EnableGzip{}); err != nil {
			return err
		}
		if err := stack.Deserialize.Add(middleware.After, &DecompressGzip{}); err != nil {
			return err
		}
		return nil
	}

	return stack.Finalize.Add(middleware.Before, &DisableGzip{})
}
