...
//
//     * Recreate
// resource record sets in the new hosted zones.
//
//     * Change the registrar's name
// servers to use the name servers for the new hosted zones.
//
//     * Monitor traffic
...
//
//     * For small numbers of hosted zones—up to a
// few hundred—it's relatively easy to create reusable delegation sets until you
// get one that has four name servers that don't overlap with any of the name
// servers in your hosted zones.
//
//     * For larger numbers of hosted zones, the
// easiest solution is to use more than one reusable delegation set.
//
//     * For
// larger numbers of hosted zones, you can also migrate hosted zones that have
// overlapping name servers to hosted zones that don't have overlapping name
...
func addOperationCreateReusableDelegationSetMiddlewares(stack *middleware.Stack, options Options) (err error) {
	err = stack.Serialize.Add(middleware.After, &awsRestxml_serializeOpCreateReusableDelegationSet{})
	if err != nil {
		return err
	}
	err = stack.Deserialize.Add(middleware.After, &awsRestxml_deserializeOpCreateReusableDelegationSet{})
	if err != nil {
		return err
	}
	...
	if err := stack.Initialize.Add(middleware.Before, newServiceMetadataMiddleware_opCreateReusableDelegationSet(options.Region)); err != nil {
		return err
	}
