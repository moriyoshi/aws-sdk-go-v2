...
type ListResolverQueryLogConfigAssociationsInput struct {
	...
	//
	//     * Error: If the
	// value of Status is FAILED, the value of Error indicates the cause:
	//
	//         *
	// DESTINATION_NOT_FOUND: The specified destination (for example, an Amazon S3
	// bucket) was deleted.
	//
	//         * ACCESS_DENIED: Permissions don't allow sending
	// logs to the destination.
	//
	//     If Status is a value other than FAILED, ERROR is
	// null.
	//
	...
	// configuration
	//
	//     * Status: The current status of the configuration. Valid
...
func addOperationListResolverQueryLogConfigAssociationsMiddlewares(stack *middleware.Stack, options Options) (err error) {
	err = stack.Serialize.Add(middleware.After, &awsAwsjson11_serializeOpListResolverQueryLogConfigAssociations{})
	if err != nil {
		return err
	}
	err = stack.Deserialize.Add(middleware.After, &awsAwsjson11_deserializeOpListResolverQueryLogConfigAssociations{})
	if err != nil {
		return err
	}
	...
	if err := stack.Initialize.Add(middleware.Before, newServiceMetadataMiddleware_opListResolverQueryLogConfigAssociations(options.Region)); err != nil {
		return err
	}
