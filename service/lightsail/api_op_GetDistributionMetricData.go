...
type GetDistributionMetricDataInput struct {
	...
	//
	//     * Requests - The total number of
	// viewer requests received by your Lightsail distribution, for all HTTP methods,
	// and for both HTTP and HTTPS requests. Statistics: The most useful statistic is
	// Sum. Unit: The published unit is None.
	//
	//     * BytesDownloaded - The number of
	// bytes downloaded by viewers for GET, HEAD, and OPTIONS requests. Statistics: The
	// most useful statistic is Sum. Unit: The published unit is None.
	//
	//     *
	// BytesUploaded  - The number of bytes uploaded to your origin by your Lightsail
	// distribution, using POST and PUT requests. Statistics: The most useful statistic
	// is Sum. Unit: The published unit is None.
	//
	//     * TotalErrorRate - The percentage
	// of all viewer requests for which the response's HTTP status code was 4xx or 5xx.
	...
	// the response's HTTP status cod was 4xx. In these cases, the client or client
	// viewer may have made an error. For example, a status code of 404 (Not Found)
	// means that the client requested an object that could not be found. Statistics:
	...
	// origin server is currently unavailable. Statistics: The most useful statistic is
...
func addOperationGetDistributionMetricDataMiddlewares(stack *middleware.Stack, options Options) (err error) {
	err = stack.Serialize.Add(middleware.After, &awsAwsjson11_serializeOpGetDistributionMetricData{})
	if err != nil {
		return err
	}
	err = stack.Deserialize.Add(middleware.After, &awsAwsjson11_deserializeOpGetDistributionMetricData{})
	if err != nil {
		return err
	}
	...
	if err := stack.Initialize.Add(middleware.Before, newServiceMetadataMiddleware_opGetDistributionMetricData(options.Region)); err != nil {
		return err
	}
