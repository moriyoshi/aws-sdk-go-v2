...
//
// By using the
// idempotent operation, you can retry a CreateBackup operation without the risk of
// creating an extra backup. This approach can be useful when an initial call fails
// in a way that makes it unclear whether a backup was created. If you use the same
...
// CreateBackup operation returns while the backup's lifecycle state is still
...
func addOperationCreateBackupMiddlewares(stack *middleware.Stack, options Options) (err error) {
	err = stack.Serialize.Add(middleware.After, &awsAwsjson11_serializeOpCreateBackup{})
	if err != nil {
		return err
	}
	err = stack.Deserialize.Add(middleware.After, &awsAwsjson11_deserializeOpCreateBackup{})
	if err != nil {
		return err
	}
	...
	if err := stack.Initialize.Add(middleware.Before, newServiceMetadataMiddleware_opCreateBackup(options.Region)); err != nil {
		return err
	}
...
func addIdempotencyToken_opCreateBackupMiddleware(stack *middleware.Stack, cfg Options) error {
	return stack.Initialize.Add(middleware.Before, &idempotencyToken_initializeOpCreateBackup{tokenProvider: cfg.IdempotencyTokenProvider})
}
