...
type DescribeTrailsInput struct {
	...
	//
	//     * If an empty list is
	// specified and IncludeShadowTrails is null or true, then information for all
	// trails in the current region and any associated shadow trails in other regions
	// is returned.
	//
...
func addOperationDescribeTrailsMiddlewares(stack *middleware.Stack, options Options) (err error) {
	err = stack.Serialize.Add(middleware.After, &awsAwsjson11_serializeOpDescribeTrails{})
	if err != nil {
		return err
	}
	err = stack.Deserialize.Add(middleware.After, &awsAwsjson11_deserializeOpDescribeTrails{})
	if err != nil {
		return err
	}
	...
	if err := stack.Initialize.Add(middleware.Before, newServiceMetadataMiddleware_opDescribeTrails(options.Region)); err != nil {
		return err
	}
